        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    <h4 class="mb-sm-0">Editar compra</h4>
                        ...
                        <form id="compraForm" action="{{ route('compras.update', $compra->id) }}" method="POST">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-control" name="supplier_id" required>
                                        ...
                                        <option value="{{ $supplier->id }}"
                                            {{ $compra->supplier_id == $supplier->id ? 'selected' : '' }}>
                                            {{ $supplier->name }}
                                        </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-control" name="tipodocumento_id" required>
                                        ...
                                        <option value="{{ $documento->id }}"
                                            {{ $compra->tipodocumento_id == $documento->id ? 'selected' : '' }}>
                                            {{ $documento->name }}
                                        </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="mt-3 mb-3">
                                ...
                                <input type="date" class="form-control" name="purchase_date"
                                    value="{{ $compra->purchase_date }}" required>
                            </div>
                                        ...
                                        <td>
                                            <select class="form-control producto" name="productos[]"
                                                ...
                                                <option value="{{ $product->id }}" data-price="{{ $product->price }}"
                                                    {{ $detalle->product_id == $product->id ? 'selected' : '' }}>
                                                    {{ $product->name }}
                                                </option>
                                                @endforeach
                                            </select>
                                        </td>
                                        <td><input type="number" class="form-control cantidad" name="cantidades[]"
                                                value="{{ $detalle->quantity }}" oninput="actualizarSubtotal(this)"
                                                min="1">
                                        </td>
                                        <td><input type="text" class="form-control precio" name="precios[]"
                                                value="{{ $detalle->unit_cost }}" readonly></td>
                                        <td><input type="text" class="form-control subtotal" name="subtotales[]"
                                                value="{{ $detalle->subtotal }}" readonly></td>
                            ...
                            <div class="mt-3">
                                ...
                                <input type="text" class="form-control" name="total_cost" id="total_cost"
                                    value="{{ $compra->total_cost }}" readonly>
                            </div>
...
function agregarProducto() {
    ...
    productos.forEach(producto => {
        productoSelect +=
            `<option value="${producto.id}" data-price="${producto.price}">${producto.name}</option>`;
    });
